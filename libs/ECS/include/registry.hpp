...
public:
    ...
    {
        _logger.log(
            Logger::Level::INFO, "Destroying entity: " + std::to_string(entity)
        );
        if (std::find(_freeIds.begin(), _freeIds.end(), entity) !=
            _freeIds.end()) {
            _logger.log(
                Logger::Level::INFO,
        ...
        for (auto& [type, set] : _componentSets) {
            try {
                ...
                _logger.log(
                    Logger::Level::INFO, "Info: " + std::string(e.what())
                );
            }
        }
    ...
    auto view()
    {
        ...
        for (auto entityId = 0; entityId < _nextEntityId; ++entityId) {
            bool hasAllComponents =
                (getComponentSet<Component>().has(entityId) && ...);
...
private:
    ...
    ComponentSet<Component>& getComponentSet()
    {
        ...
        if (_componentSets.find(typeIndex) == _componentSets.end()) {
            _componentSets[typeIndex] =
                std::make_unique<ComponentSet<Component>>();
        }
