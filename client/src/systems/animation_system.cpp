...
void GameManager::makeHoldAnimation(entt::entity& entity, sf::IntRect rectangle)
{
    ...
    if (entityAnimation.status) {
        ...
        int currentFrame =
            static_cast<int>((elapsedTime / frameTime)) % entityAnimation.totalFrames;
        entityRenderable.sprite.setTextureRect(sf::IntRect(
            rectangle.left + currentFrame * rectangle.width, rectangle.top, rectangle.width,
            rectangle.height
        ));
