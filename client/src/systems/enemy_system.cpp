...
//         if (health <= 0.0f) {
//             explosionSound.play();
//             _entityFactory.createExplosion(transformable.x - 200, transformable.y - 60);
...
//         float playerX =
//             _registry.get<TransformComponent>(_playerProfileManager.getPlayerEntity()).x;
//         float playerY =
//             _registry.get<TransformComponent>(_playerProfileManager.getPlayerEntity()).y;
//         std::pair<float, float> direction = get_direction(enemyX, enemyY, playerX, playerY);
//         _entityFactory.createEnemyProjectile(
//             direction.first, direction.second, enemyX, enemyY, 5.0f
//         );
//     }
// }
