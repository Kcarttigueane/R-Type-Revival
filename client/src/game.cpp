...
queue<string> &game::getMessages() { return _messages; };

mutex &game::getMessagesMutex() { return _messages_mutex; }

game::game(int width, int height, const std::string &windowTitle)
    : _window(VideoMode(width, height), windowTitle), _textures(assetsPath) {
    ...
    while (true) {
      ...
      if (client.stop_requested_) {
        ...
        while (!getMessages().empty()) {
          ...
          std::cout << "Sending message: " << getMessages().front()
                    << std::endl;
          getMessages().pop();
        }
      }
