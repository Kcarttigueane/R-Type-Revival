...
private:
    ...
    void process_payload() {
        ...
        std::cout << "Event from client " << remote_endpoint_.address().to_string() << ":" << remote_endpoint_.port() << ": "
...
public:
    UDPServer(boost::asio::io_context& io_context, unsigned short port)
        : socket_(io_context, boost::asio::ip::udp::endpoint(boost::asio::ip::udp::v4(), port)), io_context_(io_context) {
